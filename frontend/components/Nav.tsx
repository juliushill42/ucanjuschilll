    ...
    <>
      ...
      <aside className="fixed left-0 top-0 h-full w-64 bg-dark-900 border-r border-dark-700 z-50 hidden md:flex flex-col p-6">
        <Link href="/" className="mb-10">
          <h1 className="font-display text-2xl font-extrabold text-brand-500">JusChill 💯</h1>
          <p className="text-xs text-gray-500 mt-0.5">You can jus chill.</p>
        </Link>
        ...
        <div className="mt-auto">
          {user ? (
            ...
            >
              <div className="w-9 h-9 rounded-full bg-brand-500/20 border border-brand-500/40 overflow-hidden flex-shrink-0">
                {user.avatar_url ? (
              ...
              <div className="min-w-0">
                <p className="font-semibold text-sm text-white truncate">{user.display_name}</p>
      ...
      {/* Mobile bottom nav */}
      <nav className="fixed bottom-0 left-0 right-0 bg-dark-900 border-t border-dark-700 z-50 flex md:hidden">
        {NAV_ITEMS.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className={clsx(
              ...
              pathname === item.href ? 'text-brand-400' : 'text-gray-500'
            )}
          >
            <span className="text-xl">{item.icon}</span>
            {item.label}
        ...
        {user && (
          <Link
            ...
            className={clsx(
              'flex-1 flex flex-col items-center gap-1 py-3 text-xs font-medium transition-colors',
              pathname.startsWith('/profile') ? 'text-brand-400' : 'text-gray-500'
            )}
          >
